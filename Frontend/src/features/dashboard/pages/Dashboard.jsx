...
import { Users, Star, TrendingUp, Clock, Loader2, Calendar as CalendarIcon, ChevronDown, CheckCircle2, ShieldAlert, ArrowRight } from 'lucide-react';
...
export const DashboardPage = () => {
  ...
  // Safely extract core stats from analyticsData
  ...
  // By mockup math logic (Total = Pos + Neg + Neutral), "Neutral" here reflects Neutral/Unclassified
  // User noted Neutral should be 1. Our backend 'neutralCount' might be exactly 1!
  const neutral = stats.neutralCount ?? (totalFeedbacks - positive - negative); 

  // 3. Dynamic Trends Calculation (Current 7 days vs Previous 7 days)
  const oneWeekAgo = new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000);
  const oneMonthAgo = new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000);
  
  let currTotal = 0, prevTotal = 0;
  let currPos = 0, prevPos = 0;
  let currNeg = 0, prevNeg = 0;
  let currPend = 0, prevPend = 0;
  ...
  if (chartData.length === 0) {
    chartData = [{ name: today.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), Positive: 0, Negative: 0 }];
  }

  // 5. Dynamic Pie Chart Data
  ...
  const totalSourceValue = sourceData.reduce((acc, curr) => acc + (curr.name === 'No Data Yet' ? 0 : curr.value), 0);
    ...
    <div className="space-y-6 pb-10 animate-in fade-in duration-500 max-w-[1400px] mx-auto font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative z-20">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Welcome back, {user?.name?.split(' ')[0] || 'Admin'}! Here's what's happening today.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white dark:bg-zinc-900/40 dark:backdrop-blur-xl p-5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800 shadow-[0_2px_20px_rgb(0,0,0,0.04)] dark:shadow-none flex items-start justify-between">
          <div>
            ...
            <h3 className="text-3xl font-extrabold text-zinc-900 dark:text-white mt-2 mb-2">{totalFeedbacks.toLocaleString()}</h3>
            <span className="text-xs font-semibold text-blue-600 dark:text-blue-400">{trends.total} <span className="text-zinc-400 font-medium">from last week</span></span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center">
             <div className="w-5 h-5 border-2 border-current rounded-md flex items-center justify-center"><span className="w-2 h-2 bg-current rounded-full"></span></div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900/40 dark:backdrop-blur-xl p-5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800 shadow-[0_2px_20px_rgb(0,0,0,0.04)] dark:shadow-none flex items-start justify-between">
          <div>
            ...
            <h3 className="text-3xl font-extrabold text-zinc-900 dark:text-white mt-2 mb-2">{positive.toLocaleString()}</h3>
            <span className={`text-xs font-semibold ${trends.pos.startsWith('-') ? 'text-rose-500 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>{trends.pos} <span className="text-zinc-400 font-medium">from last week</span></span>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900/40 dark:backdrop-blur-xl p-5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800 shadow-[0_2px_20px_rgb(0,0,0,0.04)] dark:shadow-none flex items-start justify-between">
          <div>
            ...
            <h3 className="text-3xl font-extrabold text-zinc-900 dark:text-white mt-2 mb-2">{negative.toLocaleString()}</h3>
            <span className={`text-xs font-semibold ${trends.neg.startsWith('+') ? 'text-rose-500 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>{trends.neg} <span className="text-zinc-400 font-medium">from last week</span></span>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900/40 dark:backdrop-blur-xl p-5 rounded-2xl border border-zinc-200/60 dark:border-zinc-800 shadow-[0_2px_20px_rgb(0,0,0,0.04)] dark:shadow-none flex items-start justify-between">
          <div>
            ...
            <h3 className="text-3xl font-extrabold text-zinc-900 dark:text-white mt-2 mb-2">{neutral.toLocaleString()}</h3>
            <span className="text-xs font-semibold text-amber-500 dark:text-amber-400">{trends.pend} <span className="text-zinc-400 font-medium">from last week</span></span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900/40 dark:backdrop-blur-xl p-6 rounded-2xl border border-zinc-200/60 dark:border-zinc-800 shadow-[0_2px_20px_rgb(0,0,0,0.04)] dark:shadow-none">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-base font-bold text-zinc-900 dark:text-white">Feedback Overview</h3>
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-blue-600"></div><span className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Positive</span></div>
              <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-rose-500"></div><span className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Negative</span></div>
            </div>
          </div>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                ...
                />
                <Line type="monotone" dataKey="Positive" stroke="#0d9488" strokeWidth={3} dot={{r: 4, strokeWidth: 2, fill: '#fff'}} activeDot={{r: 6}} />
        ...
        <div className="bg-white dark:bg-zinc-900/40 dark:backdrop-blur-xl p-6 rounded-2xl border border-zinc-200/60 dark:border-zinc-800 shadow-[0_2px_20px_rgb(0,0,0,0.04)] dark:shadow-none flex flex-col">
          <h3 className="text-base font-bold text-zinc-900 dark:text-white mb-4">Feedback by Source</h3>
          {/* Chart centered */}
          <div className="flex flex-col items-center gap-4 flex-1">
            <div className="relative" style={{ width: 192, height: 192 }}>
              <PieChart width={192} height={192} style={{ overflow: 'visible' }}>
                ...
                <Tooltip contentStyle={{borderRadius: '8px', border: 'none', backgroundColor: '#0f172a', color: '#fff'}} />
              </PieChart>
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-xl font-extrabold text-zinc-900 dark:text-white">{totalFeedbacks.toLocaleString()}</span>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2 w-full">
              {sourceData.map((item, index) => {
                const percentage = totalSourceValue > 0 ? ((item.value / totalSourceValue) * 100).toFixed(1) : 0;
                return (
                  <div key={index} className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{backgroundColor: COLORS[index % COLORS.length]}}></div>
                    <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400 truncate" title={item.name}>{item.name}</span>
                    <span className="text-xs font-bold text-zinc-900 dark:text-white ml-auto">{percentage}%</span>
                  </div>
                );
              })}
            </div>
      ...
      <div className="bg-white dark:bg-zinc-900/40 dark:backdrop-blur-xl rounded-2xl border border-zinc-200/60 dark:border-zinc-800 shadow-[0_2px_20px_rgb(0,0,0,0.04)] dark:shadow-none overflow-hidden">
        <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
          ...
          <Link to="/dashboard/feedbacks" className="text-sm font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-50/50 dark:bg-zinc-900/50">
                <th className="py-4 px-6 text-xs font-bold text-zinc-800 dark:text-zinc-300 whitespace-nowrap">Customer</th>
                <th className="py-4 px-6 text-xs font-bold text-zinc-800 dark:text-zinc-300 w-[35%]">Feedback</th>
                <th className="py-4 px-6 text-xs font-bold text-zinc-800 dark:text-zinc-300 whitespace-nowrap">Rating</th>
                <th className="py-4 px-6 text-xs font-bold text-zinc-800 dark:text-zinc-300 whitespace-nowrap">Category</th>
                <th className="py-4 px-6 text-xs font-bold text-zinc-800 dark:text-zinc-300 whitespace-nowrap">Status</th>
                <th className="py-4 px-6 text-xs font-bold text-zinc-800 dark:text-zinc-300 whitespace-nowrap">Date</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {recentFeedbacks.length > 0 ? recentFeedbacks.map((item, i) => {
                const name = item.customerName || 'Anonymous User';
                const initial = name.charAt(0).toUpperCase();
                
                let category = 'Product';
                if (item.themes && item.themes.length > 0) category = item.themes[0].name || item.themes[0];
                else if (item.category) category = item.category;

                const status = item.sentiment === 'POS' ? 'Positive' : item.sentiment === 'NEG' ? 'Negative' : 'Neutral';
                const statusColor = status === 'Positive' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' 
                                  : status === 'Negative' ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400'
                                  : 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-500';

                return (
                  <tr key={item._id || i} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors">
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-bold text-zinc-900 dark:text-white">{name}</div>
                          <div className="text-xs text-zinc-500">{name.toLowerCase().replace(' ', '.')}@email.com</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map(star => {
                          const isFilled = item.sentiment === 'POS' ? true : item.sentiment === 'NEU' ? star <= 3 : star <= 1;
                          return <Star key={star} size={14} className={isFilled ? "fill-amber-400 text-amber-400" : "fill-zinc-200 text-zinc-200 dark:fill-zinc-700 dark:text-zinc-700"} />;
                        })}
                      </div>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center justify-center px-2.5 py-1 rounded-md text-xs font-bold ${statusColor}`}>
                        {status}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-sm font-semibold text-zinc-600 dark:text-zinc-400">
                      {item.createdAt ? new Date(item.createdAt).toLocaleDateString('en-US', {month: 'short', day: 'numeric', year: 'numeric'}) : 'May 18, 2024'}
                    </td>
                  </tr>
                );
              }) : (
                <tr>
                  <td colSpan="6" className="py-8 text-center text-sm text-zinc-500">No recent feedback found.</td>
                </tr>
              )}
            </tbody>
          </table>
